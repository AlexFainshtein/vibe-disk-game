...
- [index.html](index.html) — landing menu. Shows three buttons linking to [alex.html](alex.html), [eugene.html](eugene.html), and [zen1/zen1.html](zen1/zen1.html). Contains a single `const DEFAULT = 'default'` at the top of an inline script — change the value to `'alex'`, `'eugene'`, or `'zen1/zen1'` to forward directly to that variant on load instead of showing the menu. [alex.html](alex.html) is Alex's variant (Drift): `<body data-player="alex">` so [main.js](main.js) dynamically imports `alex-physics.js`; shows the fading "Drift" title and panel buttons (Reset, Pause, −, +, Erase).
