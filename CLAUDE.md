...
- [index.html](index.html) — production entry page; **is** Alex's variant (Drift). `<title>Drift</title>`, `<body data-player="alex">` so [main.js](main.js) dynamically imports `alex-physics.js`. Shows a big fading "Drift" title via `<div id="ui">`, plus the panel of buttons (Reset, Pause, −, +, Erase). The earlier menu-style index.html (with Alex/Eugene/Zen1 buttons and a `sessionStorage.fromMenu` hand-off) was retired when the project narrowed to shipping a single Drift variant; eugene.html and Zen1/zen1.html are still served for development comparison but no longer have entry redirects (you reach them by typing the URL).
