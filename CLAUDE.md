...
- [state.js](state.js) — shared mutable state: `canvas`, `ctx`, `params` (friction, diskRadius, frameMultiplier, bounce), `disk` (x,y,vx,vy,r), `bar` (y,prevY,vy,height), `input` (dragging, mouseBuf), `diskHistory` (last 5 positions for lag-compensated hit detection), `clickMarker`, and `clickLine`. Also owns the `resize` listener that sizes the canvas to the window.
...
- [render.js](render.js) — `draw()` renders gradient background, bar, disk shadow, disk, highlight, and debug overlays (hit boundary circle, click-to-disk line).
- [input.js](input.js) — `setupInput()` attaches pointerdown/move/up handlers. Disk dragging: grab offset, clamped to walls/bar, auto-releases with wall-parallel velocity when pointer leaves disk, scrape pips on increasing divergence. Bar dragging: grab offset, clamped so disk can't be pushed above ceiling, auto-releases when pointer leaves bar. Click hit detection checks current position plus last 5 frames (lag compensation), logs frame lag to console.
