...
- [alex-physics.js](alex-physics.js) — Alex's `update(dt)`: ticks the spring-drag controller (see below), applies friction when the spring is inactive, integrates position, bumper collision (before walls so a bumper-push that lands the disk in a wall is resolved this frame), wall bounces (×`-params.bounce`), bar-as-ceiling collision (velocity reflection + 2× bar velocity, since the disk hits the bar from below), floor-as-canvas-bottom collision, bounce chime, target tick, trail tick, and idle-reset bookkeeping. `noteFromY()` maps disk vertical position at the moment of bounce to one of 5 pentatonic notes (bar = 4 highest, floor = 0 lowest, middle band split into 3 stripes). The controller's `grabbed` event triggers `clearPause()`; its `flung` event erases the trail. On idle timeout (`IDLE_TIMEOUT` seconds of no user input), zeros the disk's velocity in place — does not reset position, bar, bumper, or trail. Also registers a `renderExtras` callback that draws the spring line from anchor to disk while held. **Owns Alex's color overrides:** sets `disk.color` and `bar.color` at module load (overriding the warm defaults in [playfield.js](playfield.js)) and defines `SPRING_COLOR` for the spring line. **Owns Alex's bar layout:** sets `bar.layout = 'top'` at module load — the bar starts at `y=0` and acts as a movable ceiling; the disk lives below it with the canvas bottom as the floor. See the constants near the top of the file for the current palette; background gradient lives in `state.js → screen.backgrounds.alex`; bumper color in [alex-bumper.js](alex-bumper.js) is kept in sync with `bar.color` so the disk-affecting "furniture" reads as one visual group.
